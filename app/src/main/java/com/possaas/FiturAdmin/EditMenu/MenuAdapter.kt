...
class MenuAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val customFont = ResourcesCompat.getFont(holder.itemView.context, R.font.plus_jakarta)

        holder.txtNamaMenu.text = menu.nama
        holder.txtNamaMenu.typeface = Typeface.create(customFont, Typeface.BOLD)

        val formatRupiah = NumberFormat.getInstance(Locale("id", "ID"))
        holder.txtHarga.text = "Rp ${formatRupiah.format(menu.harga)}"

        holder.layoutKategori.removeAllViews()
        for (kategori in menu.kategori) {
            val txtKategori = TextView(holder.itemView.context)
            txtKategori.text = kategori
            txtKategori.setTextColor(ContextCompat.getColor(holder.itemView.context, R.color.primary))
            txtKategori.textSize = 10f
            txtKategori.setPadding(22, 10, 22, 10)
            txtKategori.typeface = Typeface.create(customFont, Typeface.NORMAL)
            txtKategori.background = ContextCompat.getDrawable(holder.itemView.context, R.drawable.bg_kategori_menu)
            ...
            txtKategori.layoutParams = params
            holder.layoutKategori.addView(txtKategori)
        }
        ...
        holder.btnDelete.setOnClickListener { onDeleteClick(menu) }
    }
}